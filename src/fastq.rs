...
fn check_fastq(
    ...
    // Open input
    let fastq_fn = File::open(in_file).unwrap();
    let buf_reader = BufReader::with_capacity(1<<22, fastq_fn);
    let reader = seq_io::fastq::Reader::new(buf_reader);
...
pub fn scan_fastq(
    ...
    let n_reads = check_fastq(&infile, &flag_t)?;
...
pub fn scan_fastq_gz(
    infile: &str,
    ccsfile: &str,
    rawfile: &str,
    flag_t: &str,
) -> Result<()> {
    let n_reads = check_fastq_gz(&infile, &flag_t)?;
    info(format!("{} total reads", n_reads));
